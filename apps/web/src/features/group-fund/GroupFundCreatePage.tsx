...
const STORAGE_KEY = "tft2.group-fund.create.draft.v1";
const GROUP_FUND_CREATE_RETURN_QUERY = new URLSearchParams({
  returnTo: "/group-fund/new",
  returnLabel: "Create Group Fund Match"
}).toString();
...
const loadDraft = (): StoredGroupFundDraft | null => {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<StoredGroupFundDraft>;
    return {
...
const persistDraft = (draft: StoredGroupFundDraft) => {
  if (typeof window === "undefined") {
    return;
  }
...
const getParticipantNetClassName = (value: number) => {
  if (value > 0) {
...
export const GroupFundCreatePage = () => {
  ...
  const groupFundSummaryQuery = useGroupFundSummary({});
  const ruleSetsQuery = useQuery({
    queryKey: queryKeys.rules.list({ module: "GROUP_FUND", status: "ACTIVE", page: 1, pageSize: 100 }),
    queryFn: async () => {
      const response = await rulesApi.list({ module: "GROUP_FUND", status: "ACTIVE", page: 1, pageSize: 100 });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async () => {
      await invalidateAfterMatchCreate(queryClient, "GROUP_FUND");
      message.success("Group Fund match created successfully.");
    },
    onError: (error) => {
      setFormError(getErrorMessage(toAppError(error)));
    }
  ...
  const hasRowsMissingPlacement = slots.some((slot) => Boolean(slot.playerId) && typeof slot.tftPlacement !== "number");
  ...
  const sortedParticipantNets = useMemo(
    () => [...currentPreviewNets].sort((left, right) => right.currentNetVnd - left.currentNetVnd),
    [currentPreviewNets]
  );

  const hasPositiveParticipantNet = currentPreviewNets.some((participant) => participant.currentNetVnd > 0);
  const adjustedFundInVnd = currentPreviewNets.reduce(
    (sum, participant) => sum + (participant.currentNetVnd < 0 ? Math.abs(participant.currentNetVnd) : 0),
    0
  );
  const adjustedFundOutVnd = currentPreviewNets.reduce(
    (sum, participant) => sum + (participant.currentNetVnd > 0 ? participant.currentNetVnd : 0),
    0
  );
  const fundBeforeVnd = groupFundSummaryQuery.data?.fundBalanceVnd;
  const fundDeltaVnd = adjustedFundInVnd - adjustedFundOutVnd;
  const fundAfterVnd = typeof fundBeforeVnd === "number" ? fundBeforeVnd + fundDeltaVnd : undefined;
  ...
  return (
    <PageContainer>
      ...
      <section className="grid grid-cols-1 gap-3 xl:grid-cols-[420px_1fr]">
        <SectionCard title="Setup" bodyClassName="space-y-4">
          <div>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-800">GROUP_FUND</div>
          </div>
            ...
            {selectedRuleOption ? (
              <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Rule description</div>
                    <div
                  ...
                  <Link
                    to={`/rules/${selectedRuleOption.value}?${GROUP_FUND_CREATE_RETURN_QUERY}`}
          ...
          <div>
            ...
            />
            {requiresAdjustmentNote && !hasAdjustmentNote ? (
              <div className="mt-1 text-xs text-red-600">Note is required when participant net values are manually adjusted.</div>
            ) : null}
          </div>
        ...
        <div className="space-y-3">
          <SectionCard title="Participants" description="Choose player and placement for each participant row.">
            ...
            {hasRowsMissingPlacement ? <Alert type="warning" showIcon message="Please choose placement for all selected players." /> : null}
            {hasDuplicatePlayers ? <Alert type="error" showIcon message="Players must be unique." /> : null}
            {hasDuplicatePlacements ? <Alert type="error" showIcon message="Placements must be unique." /> : null}
          </SectionCard>
          ...
          >
            {!previewData ? <div className="text-sm text-slate-500">No preview yet. Fill participants to auto-calculate module preview.</div> : null}
              ...
              <div className="space-y-4">
                {hasPositiveParticipantNet ? (
                  <Alert
                    type="error"
                    showIcon
                    message="Participant net cannot be greater than 0. Please adjust all values to 0 or negative."
                  />
                ) : null}
                ...
                <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
                  <Card size="small">
                    ...
                    <div className="mt-1 text-sm font-semibold text-slate-900">
                      {typeof fundBeforeVnd === "number" ? formatVnd(fundBeforeVnd) : groupFundSummaryQuery.isLoading ? "Loading..." : "-"}
                    </div>
                  </Card>
                  <Card size="small">
                    <div className="text-xs text-slate-500">Fund change this match</div>
                    <div className={`mt-1 text-sm font-semibold ${fundDeltaVnd >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{formatVnd(fundDeltaVnd)}</div>
                  </Card>
                  <Card size="small">
                    <div className="text-xs text-slate-500">Estimated fund after match</div>
                    <div className="mt-1 text-sm font-semibold text-slate-900">
                      {typeof fundAfterVnd === "number" ? formatVnd(fundAfterVnd) : groupFundSummaryQuery.isLoading ? "Loading..." : "-"}
                    </div>
                  </Card>
                </div>

                <Card size="small">
                  <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Participant net snapshot</div>
                  {sortedParticipantNets.length === 0 ? (
                    <div className="text-xs text-slate-500">No participant net data.</div>
                  ) : (
                    <div className="space-y-1.5">
                      {sortedParticipantNets.map((participant) => (
                        <div key={participant.playerId} className="flex items-center justify-between gap-2 text-sm">
                          <span className="truncate text-slate-800">{participant.playerName}</span>
                          <span className={`font-semibold ${getParticipantNetClassName(participant.currentNetVnd)}`}>{formatVnd(participant.currentNetVnd)}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="flex justify-end">
                  <Button type="link" className="px-0" onClick={() => setShowManualAdjustments((previous) => !previous)}>
                    {showManualAdjustments ? "Hide loss adjustments" : "Adjust participant loss values"}
                  </Button>
                </div>
                          ...
                          <InputNumber
                            ...
                            onChange={(value) =>
                              setNetByPlayerId((previous) => ({
                                ...previous,
                                [participant.playerId]: Math.min(Number(value ?? 0), 0)
                              }))
                            }
                            addonAfter="VND"
                          />
      ...
      <Modal
        ...
        okButtonProps={{ loading: createMutation.isPending, disabled: !canCreate }}
        onOk={async () => {
      ...
      >
        <div className="space-y-3">
          {hasManualAdjustedNets ? (
            <Alert type="warning" showIcon message="Participant net values were manually adjusted from calculated results." />
          ) : null}
          {hasPositiveParticipantNet ? (
            <Alert type="error" showIcon message="Participant net cannot be greater than 0." />
          ) : null}
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Participant net snapshot</div>
            {sortedParticipantNets.length === 0 ? (
              <div className="mt-1 text-sm text-slate-500">No participant net data</div>
            ) : (
              <div className="mt-2 space-y-1.5">
                {sortedParticipantNets.map((participant) => (
                  <div key={participant.playerId} className="flex items-center justify-between gap-2 text-sm">
                    <div className="min-w-0">
                      <div className="truncate">{participant.playerName}</div>
                      <div className="text-xs text-slate-500">{`Top ${participant.tftPlacement}`}</div>
                    </div>
                    <span className={`font-semibold ${getParticipantNetClassName(participant.currentNetVnd)}`}>{formatVnd(participant.currentNetVnd)}</span>
                  </div>
                ))}
